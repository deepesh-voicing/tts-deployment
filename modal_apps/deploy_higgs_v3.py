...
VLLM_PORT = 8000
MAX_INPUTS = 64
STAGE_OVERRIDES = '{"0":{"max_num_seqs":64},"1":{"max_num_seqs":64}}'
MAX_CONTAINERS = 1
...
CACHE_PATH = "/cache"
DEPLOY_CONFIG_PATH = "/opt/tts/higgs_multimodal_qwen3.yaml"
FULL_DECODE_32_CONFIG_PATH = (
    "/opt/tts/higgs_multimodal_qwen3_full_decode_32.yaml"
)
...
image = (
    ...
    .add_local_file(
        "modal_apps/configs/higgs_multimodal_qwen3_full_decode_32.yaml",
        remote_path=FULL_DECODE_32_CONFIG_PATH,
        copy=True,
    )
    .env(
        {
...
def _build_api(deploy_config_path: str):
...
@modal.asgi_app()
def serve():
    return _build_api(DEPLOY_CONFIG_PATH)
...
@modal.concurrent(max_inputs=MAX_INPUTS)
@modal.asgi_app()
def serve_cudagraph_32():
    return _build_api(FULL_DECODE_32_CONFIG_PATH)
